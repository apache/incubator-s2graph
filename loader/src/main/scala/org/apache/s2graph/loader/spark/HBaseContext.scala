...
import org.apache.s2graph.loader.spark.HBaseRDDFunctions._
...
class HBaseContext(@transient private val sc: SparkContext,
  ...
  def streamMapPartitions[T, U: ClassTag](dstream: DStream[T],
                                          f: (Iterator[T],
                                              Connection) => Iterator[U]): DStream[U] = {
  ...
  def bulkPut[T](rdd: RDD[T], tableName: TableName, f: (T) => Put) {
    ...
    rdd.foreachPartition(
      it =>
        hbaseForeachPartition[T](broadcastedConf, it, (iterator, connection) => {
  ...
  /**
    ...
    */
  private def bulkMutation[T](rdd: RDD[T],
                              tableName: TableName,
                              f: (T) => Mutation,
                              batchSize: Integer) {
  ...
  /**
    ...
    */
  def hbaseRDD[U: ClassTag](tableName: TableName,
                            scan: Scan,
    ...
    TableMapReduceUtil.initCredentials(job)
    TableMapReduceUtil
      .initTableMapperJob(tableName, scan, classOf[IdentityTableMapper], null, null, job)
  ...
  def bulkLoad[T](rdd: RDD[T],
    ...
    val defaultCompressionStr = config.get("hfile.compression", Compression.Algorithm.NONE.getName)
     ...
     */
    def getNewWriter(family: Array[Byte],
                     conf: Configuration,
      ...
      contextBuilder.withDataBlockEncoding(
        DataBlockEncoding.valueOf(familyOptions.dataBlockEncoding)
      )
    ...
    rdd
      ...
      .hbaseForeachPartition(this, (it, conn) => {
        ...
        def close(w: StoreFile.Writer): Unit = {
          if (w != null) {
            w.appendFileInfo(
              StoreFile.BULKLOAD_TIME_KEY,
              Bytes.toBytes(System.currentTimeMillis())
            )
            w.appendFileInfo(
              StoreFile.BULKLOAD_TASK_KEY,
          ...
          case (keyFamilyQualifier, cellValue: Array[Byte]) =>
            ...
            val wl = writerMap.getOrElseUpdate(new ByteArrayWrapper(keyFamilyQualifier.family), {
              ...
              if (null == loc) {
                ...
                getNewWriter(
                  family = keyFamilyQualifier.family,
                  conf = conf,
                  favoredNodes = null,
                  fs = fs,
                  familydir = familyDir
