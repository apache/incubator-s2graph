...
class JavaHBaseContext(@transient private val jsc: JavaSparkContext,
  ...
  def foreachPartition[T](javaRdd: JavaRDD[T],
    ...
    hbaseContext.foreachPartition(javaRdd.rdd, (it: Iterator[T], conn: Connection) => {
      f.call((it, conn))
    })
  }
  ...
  def mapPartitions[T, R](
    ...
    def fn =
      (it: Iterator[T], conn: Connection) =>
        asScalaIterator(f.call((asJavaIterator(it), conn)).iterator())
  ...
  def streamMap[T, U](javaDstream: JavaDStream[T],
    ...
    JavaDStream.fromDStream(
      hbaseContext.streamMapPartitions(
        javaDstream.dstream,
        (it: Iterator[T], conn: Connection) => mp.call((it, conn))
      )(fakeClassTag[U])
    )(fakeClassTag[U])
  }
  ...
  /**
    ...
    */
  def bulkDelete[T](javaRdd: JavaRDD[T],
                    tableName: TableName,
                    f: Function[T, Delete],
  ...
  def hbaseRDD[U](tableName: TableName,
    ...
    JavaRDD.fromRDD(
      hbaseContext.hbaseRDD[U](
        tableName,
        scans,
        (v: (ImmutableBytesWritable, Result)) => f.call((v._1, v._2))
      )(fakeClassTag[U])
    )(fakeClassTag[U])
  }
