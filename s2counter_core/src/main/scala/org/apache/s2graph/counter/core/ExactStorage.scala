...
trait ExactStorage {
  ...
  // for range query
  def get(policy: Counter, items: Seq[String], timeRange: Seq[(TimedQualifier, TimedQualifier)])(
      implicit ec: ExecutionContext): Future[Seq[FetchedCounts]]

  // for query exact qualifier
  def get(policy: Counter, queries: Seq[(ExactKeyTrait, Seq[ExactQualifier])])(
      implicit ec: ExecutionContext): Future[Seq[FetchedCounts]]

  def update(policy: Counter,
