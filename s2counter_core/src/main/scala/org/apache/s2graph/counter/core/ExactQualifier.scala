...
object ExactQualifier {
  ...
  def apply(tq: TimedQualifier,
            dimKeyValues: Map[String, String]): ExactQualifier =
  ...
  def getQualifiers(intervals: Seq[IntervalUnit],
                    ts: Long,
                    dimKeyValues: Map[String, String]): Seq[ExactQualifier] =
    for {
