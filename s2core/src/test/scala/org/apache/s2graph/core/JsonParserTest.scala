...
class JsonParserTest extends FunSuite with Matchers with TestCommon with TestCommonWithModels with JSONParser {
  ...
  versions map { n =>
    ...
    test(s"aa $ver", tag) {
      val innerVal = InnerVal.withStr("abc", ver)
      val tmp = innerValToJsValue(innerVal, "string")
      println(tmp)
    }

    test(s"JsValue <-> InnerVal with dataType $ver", tag) {
      val (innerValWithDataTypes, version) = innerValsPerVersion(ver)
      testInnerValToJsValue(innerValWithDataTypes, version)
    }
  }

  def innerValsPerVersion(version: String) = {
    val innerVals = List(
    ...
    val doubleVals = if (version != VERSION1) {
      List(
