...
class GraphUtilSpec extends BenchmarkCommon {
  ...
  "GraphUtil" should {
    ...
    "test murmur hash skew2" in {
      ...
      println(
        s"Index: StartBytes ~ EndBytes\tStartShortBytes ~ EndShortBytes\tStartShort ~ EndShort\tCount\tShortCount"
      )
      sorted.foreach {
        case (idx, ((start, end), cnt)) =>
          ...
          println(
            s"$idx: $start ~ $end\t${start.take(2)} ~ ${end.take(2)}\t$startShort ~ $endShort\t$cnt\t$count"
          )
