...
trait TestCommon {
  ...
  val intVals = (Int.MinValue until Int.MinValue + 10) ++ (-129 to -126) ++ (-1 to 1) ++
      (126 to 129) ++ (Int.MaxValue - 10 until Int.MaxValue)
  val intInnerVals = intVals.map { v =>
    InnerVal.withNumber(BigDecimal(v), VERSION1)
  }

  val intInnerValsV2 = intVals.map { v =>
    InnerVal.withNumber(BigDecimal(v), VERSION2)
  }

  val stringVals = List("abc", "abd", "ac", "aca", "b")
  val stringInnerVals = stringVals.map { s =>
    InnerVal.withStr(s, VERSION1)
  }
  val stringInnerValsV2 = stringVals.map { s =>
    InnerVal.withStr(s, VERSION2)
  }
  ...
  val numInnerVals = numVals.map { n =>
    InnerVal.withLong(n.toLong, VERSION1)
  }
  val numInnerValsV2 = numVals.map { n =>
    InnerVal.withNumber(n, VERSION2)
  }
  ...
  val doubleInnerVals = doubleVals.map { d =>
    InnerVal.withDouble(d.toDouble, VERSION1)
  }
  ...
  val idxPropsLs = Seq(
    Seq((0 -> tsValSmall),
      (1 -> boolValSmall),
      (2 -> InnerVal.withStr("ac", VERSION1)),
      (toSeq -> toVal)),
    Seq((0 -> tsValSmall),
      (1 -> boolValSmall),
      (2 -> InnerVal.withStr("ab", VERSION1)),
      (toSeq -> toVal)),
    Seq((0 -> tsValSmall),
      (1 -> boolValSmall),
      (2 -> InnerVal.withStr("b", VERSION1)),
      (toSeq -> toVal)),
    Seq((0 -> tsValSmall),
      (1 -> boolValLarge),
      (2 -> InnerVal.withStr("b", VERSION1)),
  ...
  val idxPropsLsV2 = Seq(
    ...
    Seq((0 -> tsValSmallV2),
      (1 -> boolValSmallV2),
      (2 -> InnerVal.withStr("ab", VERSION2)),
      (3 -> doubleValLargeV2),
      (toSeq -> toValV2)),
    Seq((0 -> tsValSmallV2),
      (1 -> boolValSmallV2),
      (2 -> InnerVal.withStr("b", VERSION2)),
      (3 -> doubleValLargeV2),
      (toSeq -> toValV2)),
    Seq((0 -> tsValSmallV2),
      (1 -> boolValLargeV2),
