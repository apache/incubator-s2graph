...
// */
//class SourceVertexIdTest extends FunSuite with Matchers with TestCommon with TestCommonWithModels{
//  import HBaseType.{VERSION1, VERSION2}
//
//  val serializer = (idxProps: Seq[(Byte, InnerValLike)], innerVal: InnerValLike) => SourceVertexId(testColumnId, innerVal)
//  def deserializer(version: String) = (bytes: Array[Byte]) => SourceVertexId.fromBytes(bytes, 0, bytes.length, version)
//  def skipHashBytes = true
