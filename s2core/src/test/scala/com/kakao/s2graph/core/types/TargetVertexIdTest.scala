...
// */
//class TargetVertexIdTest extends FunSuite with Matchers with TestCommon with TestCommonWithModels{
//  import HBaseType.{VERSION1, VERSION2}
//
//  val serializer = (idxProps: Seq[(Byte, InnerValLike)], innerVal: InnerValLike) => TargetVertexId(testColumnId, innerVal)
//  def deserializer(version: String) = (bytes: Array[Byte]) => TargetVertexId.fromBytes(bytes, 0, bytes.length, version)
//  def skipHashBytes = false
//
