...
case class Label(id: Option[Int],
  ...
  def metas(useCache: Boolean = true): Seq[LabelMeta] =
    LabelMeta.findAllByLabelId(id.get, useCache = useCache)
  ...
  lazy val srcColumn = ServiceColumn
    .find(srcServiceId, srcColumnName)
    .getOrElse(throw ModelNotFoundException("Source column not found"))
  lazy val tgtColumn = ServiceColumn
    .find(tgtServiceId, tgtColumnName)
    .getOrElse(throw ModelNotFoundException("Target column not found"))
...
//  lazy val metas = metas(useCache = true)
  ...
  lazy val labelMetaMap =
    (labelMetas ++ LabelMeta.reservedMetas).map(m => m.seq -> m).toMap
  ...
  def innerValsWithTsToProps(props: Map[LabelMeta, InnerValLikeWithTs],
    ...
    if (selectColumns.isEmpty) {
      ...
      for {
        ...
        val v = props
          .get(labelMeta)
          .orElse(metaPropsDefaultMapInner.get(labelMeta))
          .get
