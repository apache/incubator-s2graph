...
object ColumnMeta extends Model[ColumnMeta] {
  ...
  val lastModifiedAtColumn =
    ColumnMeta(Some(0), 0, "lastModifiedAt", lastModifiedAtColumnSeq, "long")
  ...
  def findById(id: Int)(
      implicit session: DBSession = AutoSession): ColumnMeta = {
  ...
  def findAllByColumn(columnId: Int, useCache: Boolean = true)(
      implicit session: DBSession = AutoSession): Seq[ColumnMeta] = {
    //    val cacheKey = s"columnId=$columnId"
    val cacheKey = "columnId=" + columnId
    if (useCache) {
      withCaches(cacheKey)(
        sql"""select *from column_metas where column_id = ${columnId} order by seq ASC""".map {
          rs =>
            ColumnMeta(rs)
        }.list.apply())
    } else {
  ...
  def findByName(columnId: Int, name: String)(
      implicit session: DBSession = AutoSession): Option[ColumnMeta] = {
    //    val cacheKey = s"columnId=$columnId:name=$name"
    val cacheKey = "columnId=" + columnId + ":name=" + name
    withCache(cacheKey)(
      sql"""select * from column_metas where column_id = ${columnId} and name = ${name}""".map {
        rs =>
          ColumnMeta(rs)
      }.single.apply())
  }
  ...
  def insert(columnId: Int, name: String, dataType: String)(
    ...
    select ${columnId}, ${name}, ${seq}, ${dataType}""".updateAndReturnGeneratedKey
        .apply()
    }
  }

  def findOrInsert(columnId: Int, name: String, dataType: String)(
      implicit session: DBSession = AutoSession): ColumnMeta = {
  ...
  def delete(id: Int)(implicit session: DBSession = AutoSession): Unit = {
    ...
    val cacheKeys =
      List(s"id=$id", s"columnId=$columnId:name=$name", s"colunmId=$columnId")
