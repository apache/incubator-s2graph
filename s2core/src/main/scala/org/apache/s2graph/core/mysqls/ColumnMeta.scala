...
object ColumnMeta extends Model[ColumnMeta] {
  ...
  def findById(id: Int)(implicit session: DBSession = AutoSession): ColumnMeta = {
    ...
    val cacheKey = "id=" + id
    withCache(cacheKey) {
      sql"""select * from column_metas where id = ${id}"""
        .map { rs =>
          ColumnMeta(rs)
        }
        .single
  ...
  def findByName(columnId: Int,
                 name: String)(implicit session: DBSession = AutoSession): Option[ColumnMeta] = {
  ...
  def insert(columnId: Int, name: String, dataType: String)(implicit session: DBSession =
                                                              AutoSession): Unit = {
  ...
  def findOrInsert(columnId: Int, name: String, dataType: String)(implicit session: DBSession =
