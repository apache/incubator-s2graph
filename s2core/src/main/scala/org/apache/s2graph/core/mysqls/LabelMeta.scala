...
object LabelMeta extends Model[LabelMeta] {
  ...
  /** Note: DegreeSeq should not be included in serializer/deserializer.
    ...
    * */
  def isValidSeq(seq: Byte): Boolean =
    seq >= 0 && seq <= countSeq // || seq == fromHashSeq

  def isValidSeqForAdmin(seq: Byte): Boolean =
    seq > 0 && seq < countSeq // || seq == fromHashSeq
  ...
  def insert(labelId: Int, name: String, defaultValue: String, dataType: String)(
    ...
    select ${labelId}, ${name}, ${seq}, ${defaultValue}, ${dataType}""".updateAndReturnGeneratedKey
        .apply()
    } else {
  ...
  def delete(id: Int)(implicit session: DBSession = AutoSession): Unit = {
    ...
    val cacheKeys =
      List(s"id=$id", s"labelId=$labelId", s"labelId=$labelId:name=$name")
