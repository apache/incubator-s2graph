...
object S2Graph {
  ...
  def alreadyVisitedVertices(
      edgeWithScoreLs: Seq[EdgeWithScore]
  ): Map[(LabelWithDirection, S2Vertex), Boolean] = {
    val vertices = for {
      edgeWithScore <- edgeWithScoreLs
      edge = edgeWithScore.edge
      vertex = if (edge.labelWithDir.dir == GraphUtil.directions("out")) edge.tgtVertex
      else edge.srcVertex
  ...
  def convertEdges(queryParam: QueryParam, edge: S2Edge, nextStepOpt: Option[Step]): Seq[S2Edge] =
    for {
      convertedEdge <- queryParam.edgeTransformer.transform(queryParam, edge, nextStepOpt)
      if !edge.isDegree
    } yield convertedEdge
  ...
  def processDuplicates[T](queryParam: QueryParam, duplicates: Seq[(FilterHashKey, T)])(
    ...
    if (queryParam.label.consistencyLevel != "strong") {
      ...
      queryParam.duplicatePolicy match {
        ...
        case _ =>
          val scoreSum = duplicates.foldLeft(0.0) {
            case (prev, current) => prev + ev.score(current._2)
          }
          val (headFilterHashKey, headEdgeWithScore) = duplicates.head
  ...
  def toHashKey(queryParam: QueryParam,
                edge: S2Edge,
                isDegree: Boolean): (HashKey, FilterHashKey) = {
    ...
    queryResultLsFuture.map { queryRequestWithResultLs =>
      ...
      if (queryRequestWithResultLs.isEmpty) StepResult.Empty.copy(failCount = failCount)
        ...
        if (shouldBuildInnerResults) {
          val _results = buildResult(q, stepIdx, currentStepResults, parentEdges) {
            (edgeWithScore, propsSelectColumns) =>
              edgeWithScore
          }
...
//            val newEdge = edge.copy(propsWithTs = mergedPropsWithTs)
          ...
          val ordered =
            if (queryOption.groupBy.keys.isEmpty) StepResult.orderBy(queryOption, results) else Nil

          /** process grouped list */
          val grouped =
            if (queryOption.groupBy.keys.isEmpty) Nil
            else {
              val agg = new scala.collection.mutable.HashMap[
                StepResult.GroupByKey,
                (Double, StepResult.Values)
              ]()
              results
  ...
  private def buildResult[T](
      query: Query,
      stepIdx: Int,
      stepResultLs: Seq[(QueryRequest, StepResult)],
      parentEdges: Map[VertexId, Seq[EdgeWithScore]]
  )(createFunc: (EdgeWithScore, Seq[LabelMeta]) => T)(implicit ev: WithScore[T]): ListBuffer[T] = {
    ...
    stepResultLs.foreach {
      case (queryRequest, stepInnerResult) =>
        val queryParam = queryRequest.queryParam
    ...
    if (numOfDuplicates == 0) {
      ...
      for {
        ...
        processDuplicates(queryParam, duplicates(hashKey)).foreach {
          case (_, duplicate) =>
...
class S2Graph(_config: Config)(implicit val ec: ExecutionContext) extends Graph {
  ...
  val storagePool: scala.collection.mutable.Map[String, Storage[_, _]] = {
    ...
    val configs = (labelConfigs ++ serviceConfigs)
      .map { conf =>
  ...
  val queryFutureCache = new DeferCache[StepResult, Promise, Future](
    parseCacheConfig(config, "query."),
    empty = StepResult.Empty
  )
  ...
  def checkEdges(edges: Seq[S2Edge]): Future[StepResult] = {
    val futures = for {
      ...
      fetchSnapshotEdge(edge).map {
        case (queryParam, edgeOpt, kvOpt) =>
          ...
          q.steps.zipWithIndex.foldLeft(Future.successful(startStepInnerResult)) {
            case (prevStepInnerResultFuture, (step, stepIdx)) =>
              for {
                ...
                currentStepInnerResult <- fetchStep(
                  q,
                  stepIdx,
                  prevStepInnerResult,
                  buildLastStepInnerResult
                )
              } yield {
  ...
  def fetchStep(orgQuery: Query,
    ...
    if (stepInnerResult.isEmpty) Future.successful(StepResult.Empty)
      ...
      val (sums, grouped) = edgeWithScoreLs.foldLeft(initial) {
        case ((sum, group), edgeWithScore) =>
      ...
      filterEdges(
        ...
        queryRequests,
        fetchedLs,
        orgQuery.steps(stepIdx).queryParams,
        alreadyVisited,
        buildLastStepInnerResult,
        prevStepTgtVertexIdEdges
      )(ec)
    }
  ...
  def fetches(queryRequests: Seq[QueryRequest],
    ...
    val aggFuture = requestsPerLabel.foldLeft(Future.successful(Map.empty[Int, StepResult])) {
      case (prevFuture, (label, reqWithIdxs)) =>
        for {
          prev <- prevFuture
    ...
    aggFuture.map { agg =>
      agg.toSeq.sortBy(_._1).map(_._2)
    }
  }
    ...
    storage.fetchSnapshotEdgeKeyValues(queryRequest).map { kvs =>
      val (edgeOpt, kvOpt) =
        if (kvs.isEmpty) (None, None)
          ...
          val snapshotEdgeOpt =
            storage.toSnapshotEdge(kvs.head, queryRequest, isInnerCall = true, parentEdges = Nil)
  ...
  def getVertices(vertices: Seq[S2Vertex]): Future[Seq[S2Vertex]] = {
    ...
    val futures = verticesWithIdx.groupBy { case (v, idx) => v.service }.map {
      case (service, vertexGroup) =>
  ...
  def deleteAllAdjacentEdges(srcVertices: Seq[S2Vertex],
    ...
    val queries = for {
      ...
      val queryParam = QueryParam(
        ...
        direction = GraphUtil.fromDirection(dir),
        offset = 0,
        limit = DeleteAllFetchSize,
    ...
    val retryFuture = Extensions
      .retryOnSuccess(DeleteAllFetchCount) {
        ...
        case HBaseType.VERSION3 | HBaseType.VERSION4 =>
          if (label.consistencyLevel == "strong") {
            ...
            /**
              ...
              */
            mutateEdges(deleteStepInnerResult.edgeWithScores.map(_.edge), withWait = true)
        ...
        val copiedEdge = label.consistencyLevel match {
          case "strong" =>
            edge.copyEdge(
              ...
              version = requestTs,
              propsWithTs = S2Edge.propsToState(edge.updatePropsWithTs()),
              ts = requestTs
            )
          case _ =>
            edge.copyEdge(
...
//          edgeWithScore.edge.copy(op = newOp, version = newVersion, propsWithTs = newPropsWithTs)
  ...
  def mutateEdges(edges: Seq[S2Edge], withWait: Boolean = false): Future[Seq[Boolean]] = {
    ...
    val (strongEdges, weakEdges) =
      edgeWithIdxs.partition {
        case (edge, idx) =>
          val e = edge
          e.innerLabel.consistencyLevel == "strong" && e.op != GraphUtil.operations("insertBulk")
      }

    val weakEdgesFutures =
      weakEdges.groupBy { case (edge, idx) => edge.innerLabel.hbaseZkAddr }.map {
        case (zkQuorum, edgeWithIdxs) =>
          val futures = edgeWithIdxs.groupBy(_._1.innerLabel).map {
            case (label, edgeGroup) =>
              ...
              val mutations = edges.flatMap { edge =>
                ...
                storage.buildVertexPutsAsync(edge) ++ storage
                  .indexedEdgeMutations(edgeUpdate) ++ storage
                  .snapshotEdgeMutations(edgeUpdate) ++ storage.increments(edgeUpdate)
              }
    ...
    val (strongDeleteAll, strongEdgesAll) = strongEdges.partition {
      case (edge, idx) => edge.op == GraphUtil.operations("deleteAll")
    }

    val deleteAllFutures = strongDeleteAll.map {
      case (edge, idx) =>
        deleteAllAdjacentEdges(
          Seq(edge.srcVertex),
          Seq(edge.innerLabel),
          edge.labelWithDir.dir,
          edge.ts
        ).map(idx -> _)
    }

    val strongEdgesFutures = strongEdgesAll.groupBy { case (edge, idx) => edge.innerLabel }.map {
      case (label, edgeGroup) =>
        val edges = edgeGroup.map(_._1)
        val idxs = edgeGroup.map(_._2)
        val storage = getStorage(label)
  ...
  def mutateVertices(vertices: Seq[S2Vertex], withWait: Boolean = false): Future[Seq[Boolean]] = {
    ...
    val futures = verticesWithIdx.groupBy { case (v, idx) => v.service }.map {
      case (service, vertexGroup) =>
        getStorage(service)
          .mutateVertices(vertexGroup.map(_._1), withWait)
          .map(_.zip(vertexGroup.map(_._2)))
    }
    Future.sequence(futures).map { ls =>
      ls.flatten.toSeq.sortBy(_._2).map(_._1)
    }
  ...
  def incrementCounts(edges: Seq[S2Edge], withWait: Boolean): Future[Seq[(Boolean, Long, Long)]] = {
    ...
    val futures = edgesWithIdx.groupBy { case (e, idx) => e.innerLabel }.map {
      case (label, edgeGroup) =>
        getStorage(label)
          .incrementCounts(edgeGroup.map(_._1), withWait)
          .map(_.zip(edgeGroup.map(_._2)))
    }
    Future.sequence(futures).map { ls =>
      ls.flatten.toSeq.sortBy(_._2).map(_._1)
    }
  ...
  def toGraphElement(s: String,
                     labelMapping: Map[String, String] = Map.empty): Option[GraphElement] =
    Try {
  ...
  def toEdge(parts: Array[String]): Option[S2Edge] =
    Try {
      val (ts, operation, logType, srcId, tgtId, label) =
        (parts(0), parts(1), parts(2), parts(3), parts(4), parts(5))
      val props =
        if (parts.length >= 7) {
          fromJsonToProperties(Json.parse(parts(6)).asOpt[JsObject].getOrElse(Json.obj()))
  ...
  def toVertex(parts: Array[String]): Option[S2Vertex] =
    Try {
      val (ts, operation, logType, srcId, serviceName, colName) =
        (parts(0), parts(1), parts(2), parts(3), parts(4), parts(5))
      val props =
        if (parts.length >= 7) {
          fromJsonToProperties(Json.parse(parts(6)).asOpt[JsObject].getOrElse(Json.obj()))
  ...
  def toEdge(srcId: Any,
    ...
    val srcVertexIdInnerVal =
      toInnerVal(srcId.toString, label.srcColumn.columnType, label.schemaVersion)
    val tgtVertexIdInnerVal =
      toInnerVal(tgtId.toString, label.tgtColumn.columnType, label.schemaVersion)

    val srcVertex =
      newVertex(SourceVertexId(label.srcColumn, srcVertexIdInnerVal), System.currentTimeMillis())
    val tgtVertex =
      newVertex(TargetVertexId(label.tgtColumn, tgtVertexIdInnerVal), System.currentTimeMillis())
    val dir = GraphUtil
      .toDir(direction)
    ...
    val op = GraphUtil
      .toOp(operation)
      .getOrElse(throw new RuntimeException(s"$operation is not supported."))

    new S2Edge(this, srcVertex, tgtVertex, label, dir, op = op, version = ts)
      .copyEdgeWithState(propsWithTs)
  }
  ...
  def toVertex(serviceName: String,
    ...
    val service = Service
      ...
      .getOrElse(throw new RuntimeException(s"$serviceName is not found."))
    val column = ServiceColumn
      .find(service.id.get, columnName)
      .getOrElse(throw new RuntimeException(s"$columnName is not found."))
    val op = GraphUtil
      .toOp(operation)
      .getOrElse(throw new RuntimeException(s"$operation is not supported."))

    val srcVertexId =
      VertexId(column, toInnerVal(id.toString, column.columnType, column.schemaVersion))
  ...
  private[core] def newSnapshotEdge(srcVertex: S2Vertex,
    ...
    val snapshotEdge = new SnapshotEdge(
      this,
      srcVertex,
      tgtVertex,
      label,
  ...
  private[core] def addEdgeInner(srcVertex: S2Vertex,
                                 ...
                                 operation: String = "insert"): S2Edge =
    Await.result(
      addEdgeInnerAsync(srcVertex, tgtVertex, labelName, direction, props, ts, operation),
      WaitTimeout
    )
...
//    if (tgtVertex.id.column != label.tgtColumnWithDir(dir)) throw new RuntimeException(s"tgtVertex's column[${tgtVertex.id.column}] is not matched to label's tgtColumn[${label.tgtColumnWithDir(dir)}")
    ...
    val edge =
      newEdge(srcVertex, tgtVertex, label, dir, op = op, version = ts, propsWithTs = propsWithTs)
  ...
  /**
    ...
    * @return
    */
  def newVertexId(service: Service, column: ServiceColumn, id: Any): VertexId = {
  ...
  def getVertex(vertexId: VertexId): Option[S2Vertex] = {
    ...
    Await.result(getVertices(Seq(v)).map { vertices =>
      vertices.headOption
    }, WaitTimeout)
  }

  def fetchEdges(vertex: S2Vertex,
                 labelNames: Seq[String],
                 direction: String = "out"): util.Iterator[Edge] =
    Await.result(fetchEdgesAsync(vertex, labelNames, direction), WaitTimeout)

  def fetchEdgesAsync(vertex: S2Vertex,
                      labelNames: Seq[String],
                      direction: String = "out"): Future[util.Iterator[Edge]] = {
  ...
  override def vertices(vertexIds: AnyRef*): util.Iterator[structure.Vertex] = {
    val fetchVertices = vertexIds.lastOption
      .map { lastParam =>
  ...
  override def addVertex(kvs: AnyRef*): structure.Vertex = {
    ...
    val serviceColumnNames = kvsMap
      .getOrElse(
        T.label.toString,
        throw new RuntimeException("ServiceName::ColumnName is required.")
      )
      .toString
