 ...
 */
object InnerValV1 {
//  val defaultVal = new InnerValV1(None, None, None)
  ...
  def maxIdVal(dataType: String) = {
    dataType match {
      case "string" => InnerValV1.withStr((0 until (Byte.MaxValue - stringLenOffset)).map("~").mkString)
      case "long" => InnerValV1.withLong(Long.MaxValue)
      case "bool" => InnerValV1.withBoolean(true)
      case _ => throw IllegalDataTypeException(dataType)
    }
  }
  def minIdVal(dataType: String) = {
    dataType match {
      case "string" => InnerValV1.withStr("")
      case "long" => InnerValV1.withLong(1)
      case "bool" => InnerValV1.withBoolean(false)
      case _ => throw IllegalDataTypeException(dataType)
    }
  }
  def apply(bytes: Array[Byte], offset: Int): InnerValV1 = {
    var pos = offset
    //
    val len = bytes(pos)
    ...
    InnerValV1(longV, strV, boolV)
  }

  def withLong(l: Long): InnerValV1 = {
    //      if (l < 0) throw new IllegalDataRangeException("value shoudl be >= 0")
    InnerValV1(Some(l), None, None)
  }
  def withStr(s: String): InnerValV1 = {
    InnerValV1(None, Some(s), None)
  }
  def withBoolean(b: Boolean): InnerValV1 = {
    InnerValV1(None, None, Some(b))
  }
  /**
   * In natural order
...
case class InnerValV1(longV: Option[Long], strV: Option[String], boolV: Option[Boolean]) extends InnerVal(null) {
  import InnerValV1._
  override val value = (longV, strV, boolV) match {
  ...
  override def compare(other: InnerVal) = {
