...
object VertexController extends Controller {
  ...
  def tryMutates(jsValue: JsValue,
    ...
    if (!Config.IS_WRITE_SERVER) Future.successful(Unauthorized)
      ...
      try {
        val vertices = requestParser
          .toVertices(jsValue, operation, serviceNameOpt, columnNameOpt)
        ...
        if (verticesToStore.isEmpty) {
          ...
          if (withWait) {
            ...
            val rets = verticesToStore.map { vertex =>
              QueueActor.router ! vertex; true
            }
  ...
  def deletesAll(): Action[JsValue] = withHeaderAsync(jsonParser) { request =>
