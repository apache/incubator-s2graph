...
object S2Parse extends BodyParsers {
  ...
  def tolerantJson(maxLength: Int): BodyParser[JsValue] =
    tolerantBodyParser[JsValue]("json", maxLength, "Invalid Json") { (request, bytes) =>
      ...
      // and states that whether that's UTF-8, UTF-16 or UTF-32 can be auto detected by reading
      // the first two bytes. So we ignore the declared charset and don't decode,
      // we passing the byte array as is because Jackson supports auto detection.
      Json.parse(bytes)
    }
