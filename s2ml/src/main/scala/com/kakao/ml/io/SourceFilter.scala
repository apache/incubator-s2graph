...
case class SourceFilterParams(
    ...
    itemUpperBound: Option[Long]) extends Params
...
class SourceFilter(params: SourceFilterParams)
  ...
  override def processBlock(sqlContext: SQLContext, input: SourceData): SourceData = {
    ...
    logInfo(s"userLowerBound: $userLowerBound")
    logInfo(s"userUpperBound: $userUpperBound")
    logInfo(s"itemLowerBound: $itemLowerBound")
    logInfo(s"itemUpperBound: $itemUpperBound")
